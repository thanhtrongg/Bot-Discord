...
const { ActivityType, ChannelType, Collection, GatewayIntentBits, Partials } =
  Discord;
...
bot.on("messageCreate", async (message) => {
  ...
  if (data.toggle == 1) {
    ...
    if (hasLeveledUp) {
      ...
      bot.channels.cache
        .get("966237517059862610")
        ?.send(
          `${message.author}, Chúc mừng! Bạn đã lên level **${user.level}**!`,
        );

      if (user.level == 1) {
        let role = message.guild.roles.cache.find(
          (role) => role.name == "Level 1",
        );
        if (!role)
      ...
      if (user.level == 15) {
        let role = message.guild.roles.cache.find(
          (role) => role.name == "Anh Hùng Bàn Phím",
        );
        if (!role)
          await message.guild.roles
            .create({
      ...
      if (command.cooldown) {
        if (Timeout.has(`${command.name}${message.author.id}`))
          return message.channel.send(
            `Please Wait \`${ms(Timeout.get(`${command.name}${message.author.id}`) - Date.now(), { long: true })}\` Before using this command again!`,
          );
  ...
  if (usersMap.has(message.author.id)) {
    ...
    if (difference > DIFF) {
      ...
      if (parseInt(msgCount) === LIMIT) {
        ...
        bot.channels.cache
          .get("966237517059862610")
          .send(`${message.author}, sống chậm lại nào!`);
        setTimeout(() => {
          message.member.roles.remove(muterole).catch((error) => {
            console.warn(
              `Cannot remove role ${muterole.name} from ${message.author.tag}: ${error.message}`,
            );
          });
          bot.channels.cache
            .get("966237517059862610")
            .send(`${message.author} đã được unmute!`);
        }, TIME);
      } else {
...
bot.distube
  .on("playSong", (queue, song) =>
    queue.textChannel?.send(
      `Nghe bài \`${song.name}\` - \`${song.formattedDuration || song.formatDuration?.() || "unknown"}\`\nThành viên yêu cầu: ${song.user}`,
    ),
  )
  .on("addSong", (queue, song) =>
    queue.textChannel?.send(
      `Thêm bài ${song.name} - \`${song.formattedDuration || song.formatDuration?.() || "unknown"}\` vào hàng đợi bởi ${song.user}`,
    ),
  )
...
async function addRoleSafely(member, role) {
  ...
  try {
    ...
    console.warn(
      `Cannot add role ${role.name} to ${member.user.tag}: ${error.message}`,
    );
    return false;
  }
}
